...
def load_labels(root: str):
    ...
    with open(path, 'r') as gt_file:
        labels = [int(line.rstrip().split()[0]) for line in gt_file]
    return labels
...
def load_imgs(root: str):
    ...
    imgs = [cv.imread(os.path.abspath(img)) for img in glob.glob(path + '/*jpg')]
    return imgs
...
def split_data(imgs: list, labels: list):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        imgs, labels, test_size=0.2, random_state=42, stratify=True
        )
